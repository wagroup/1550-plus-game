...
export default function PlayPage() {
  ...
  const { state, error, connected } = useRoomPoll(roomCode, 'public', {
  ...
  useEffect(() => {
    ...
    studentJoin(roomCode, { sessionToken: session.sessionToken })
      .then(() => setJoined(true))
      .catch((e) => {
        clearStudentSession(roomCode);
        setFatal(e instanceof Error ? e.message : 'Unable to reconnect to this game.');
      });
  ...
  // Detect kick: participant no longer present anywhere once we know we've joined.
  useEffect(() => {
    if (!state || !session || !joined || fatal) return;
    const ids = new Set<string>([
      ...state.teams.A.members.map((m) => m.id),
      ...state.teams.B.members.map((m) => m.id),
      ...state.unassigned.map((m) => m.id),
    ]);
    if (!ids.has(session.participantId)) {
      clearStudentSession(roomCode);
      setFatal('You were removed from the game by the teacher.');
    }
  }, [state, session, joined, fatal, roomCode]);
  ...
  const buzz = useCallback(() => {
    ...
    studentBuzz(roomCode, session.sessionToken).then((res) => {
