          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                href={item.to}
                className={`flex items-center gap-3 rounded-xl px-4 py-2.5 font-semibold transition-colors ${
      ...
      <div className="flex-1 min-w-0">
        <header className="md:hidden flex items-center justify-between bg-navy text-white px-4 py-3">
          ...
          <div className="flex gap-2">
            {navItems.map((item) => (
              <Link key={item.to} href={item.to} className="text-xl" title={item.label}>
                {item.icon}
              </Link>
            ))}
          </div>
