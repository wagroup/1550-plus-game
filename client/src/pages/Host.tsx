...
import type { GameState, Member, TeamId } from '../types';
...
export default function Host() {
  ...
  useEffect(() => {
    ...
    const join = () => {
      socket.emit('teacher:join', { token: getToken(), roomCode }, (res: any) => {
  ...
  const action = useCallback((type: string, payload?: any) => {
    getSocket().emit('teacher:action', { type, payload }, (res: any) => {
      if (res?.error) setError(res.error);
      else setError(null);
    });
  }, []);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/dashboard" className="text-slate-500 font-semibold hover:text-navy">Dashboard</Link>
          </div>
        </div>
  ...
  return (
    <div className="min-h-full bg-navy text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="flex gap-4">
        <Button onClick={() => navigate(`/reports/${state.id}`)}>📊 View Full Report</Button>
