...
import { Button, Card, Field, inputClass, ErrorBanner } from '../components/ui';
import { api } from '../api';
import type { GameSettings, QuestionSet } from '../types';

const TEAM_ICONS = ['🐯', '🦅', '🦁', '🐺', '🦈', '🐉', '🚀', '⚡', '🔥', '🌟', '🧠', '🏆'];
const TEAM_COLORS = ['#2563EB', '#F97316', '#16A34A', '#DC2626', '#9333EA', '#0891B2', '#DB2777', '#CA8A04'];
...
export default function CreateGame() {
  ...
  const [teamA, setTeamA] = useState({ name: 'Team Tigers', color: '#2563EB', icon: '🐯' });
  const [teamB, setTeamB] = useState({ name: 'Team Eagles', color: '#F97316', icon: '🦅' });
  ...
  async function create() {
    ...
    try {
      ...
      navigate(`/host/${data.game.roomCode}`);
    } catch (e: any) {
      setError(e.message);
      setCreating(false);
    }
      ...
      <div className="flex gap-2 mb-8">
        {steps.map((label, i) => (
          <div key={label} className={`flex-1 text-center py-2 rounded-xl font-semibold text-sm ${
            step === i + 1 ? 'bg-primary text-white' : step > i + 1 ? 'bg-blue-100 text-primary' : 'bg-slate-100 text-slate-400'
          }`}>
            {i + 1}. {label}
          </div>
        ))}
      ...
      {step === 1 && (
        <Card className="p-6 space-y-5 max-w-2xl">
          ...
          <Field label="Question set">
            {sets.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-300 p-6 text-center text-slate-500">
                ...
                <Link to="/question-sets" className="text-primary font-semibold hover:underline">Create a question set first →</Link>
              </div>
            ) : (
                ...
                {sets.map((s) => (
                  <button key={s.id} type="button" onClick={() => setQuestionSetId(s.id)}
                    className={`w-full text-left rounded-xl border-2 px-4 py-3 transition-colors cursor-pointer ${
                      questionSetId === s.id ? 'border-primary bg-blue-50' : 'border-slate-200 hover:border-slate-300'
                    }`}>
                    <span className="font-bold">{s.title}</span>
                    <span className="text-sm text-slate-500 ml-2">{s.questions.length} questions{s.subject ? ` · ${s.subject}` : ''}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex justify-end">
            <Button onClick={() => setStep(2)} disabled={!questionSetId} className="px-8">Continue</Button>
          </div>
        </Card>
      )}
      ...
      {step === 2 && (
        <div className="grid md:grid-cols-2 gap-6 max-w-3xl">
          {([['A', teamA, setTeamA], ['B', teamB, setTeamB]] as const).map(([id, team, setTeam]) => (
            <Card key={id} className="p-6 border-t-8" style={{ borderTopColor: team.color }}>
              <h3 className="font-extrabold text-lg mb-4">Team {id}</h3>
              <div className="space-y-4">
                <Field label="Team name">
                  <input className={inputClass} value={team.name} maxLength={30}
                    onChange={(e) => setTeam({ ...team, name: e.target.value })} />
                </Field>
                <Field label="Team icon">
                  <div className="flex flex-wrap gap-2">
                    {TEAM_ICONS.map((icon) => (
                      <button key={icon} type="button" onClick={() => setTeam({ ...team, icon })}
                        className={`text-2xl rounded-lg p-1.5 cursor-pointer ${team.icon === icon ? 'bg-blue-100 ring-2 ring-primary' : 'hover:bg-slate-100'}`}>
                        {icon}
                      </button>
                    ))}
                  </div>
      ...
      {step === 3 && (
        <Card className="p-6 space-y-5 max-w-2xl">
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Question reading time (seconds)">
              <input className={inputClass} type="number" min={0} max={60} value={settings.readingSeconds}
                onChange={(e) => setSettings({ ...settings, readingSeconds: Math.max(0, +e.target.value) })} />
            </Field>
            <Field label="Team discussion time (seconds)">
              <input className={inputClass} type="number" min={0} max={120} value={settings.discussionSeconds}
                onChange={(e) => setSettings({ ...settings, discussionSeconds: Math.max(0, +e.target.value) })} />
            </Field>
            <Field label="Points for a correct answer">
              <input className={inputClass} type="number" min={1} max={10} value={settings.pointsCorrect}
                onChange={(e) => setSettings({ ...settings, pointsCorrect: Math.max(1, +e.target.value) })} />
            </Field>
            <Field label="Wrong-answer penalty (0 = none)">
              <input className={inputClass} type="number" min={0} max={10} value={settings.penaltyWrong}
                onChange={(e) => setSettings({ ...settings, penaltyWrong: Math.max(0, +e.target.value) })} />
            </Field>
          </div>
          <Field label="After a wrong answer">
            <select className={inputClass} value={settings.secondChance}
          ...
          <div className="space-y-3">
            {([
              ...
              <label key={key} className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" className="w-5 h-5 accent-blue-600" checked={settings[key]}
          ...
          <div className="flex justify-between pt-2">
            ...
            <Button onClick={create} disabled={creating} className="px-8 text-lg">
