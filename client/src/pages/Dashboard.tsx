...
import { api, getStoredTeacher } from '../api';
import type { GameDef, GameReport, QuestionSet } from '../types';
  ...
  return (
    <TeacherLayout title={`Welcome back, ${teacher?.name?.split(' ')[0] || 'Teacher'} 👋`}>
      ...
      <div className="flex flex-wrap gap-4 mb-10">
        <Button className="text-lg px-6 py-3" onClick={() => navigate('/create-game')}>🎮 Create New Game</Button>
        <Button variant="secondary" className="text-lg px-6 py-3" onClick={() => navigate('/question-sets')}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {activeGames.map((g) => (
              <Card key={g.id} className="p-5 flex items-center justify-between border-primary border-2">
                ...
                <Button onClick={() => navigate(`/host/${g.roomCode}`)}>Open Host View</Button>
              </Card>
            ))}
          </div>
        </section>
              ...
              return (
                <Card key={g.id} className="p-4 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {report && (
                      <Link to={`/reports/${g.id}`} className="text-primary font-semibold text-sm hover:underline">
                        View report
                      </Link>
                    )}
                    {g.live && <Button variant="secondary" onClick={() => navigate(`/host/${g.roomCode}`)}>Host</Button>}
                  </div>
                </Card>
              );
            })}
